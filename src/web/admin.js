...
function renderPlacePhotos(){const section=$('#place-photo-section'),grid=$('#place-photo-grid');section.hidden=!placePhotos.length;grid.replaceChildren(...placePhotos.map((photo,index)=>{const card=document.createElement('article');card.className='place-photo';const credits=photo.credits.length?photo.credits.map(credit=>`<a href="${esc(credit.url)}" target="_blank" rel="noopener noreferrer">${esc(credit.name)}</a>`).join(' · '):'Google Maps';card.innerHTML=`<img src="${esc(photo.url)}" alt="Google Maps 地点图片 ${index+1}"><div class="place-photo-copy"><span>${credits}</span><a href="${esc(photo.source)}" target="_blank" rel="noopener noreferrer">在 Google Maps 中查看 ↗</a></div>`;return card;}));}
function renderPhotos(){const grid=$('#photo-grid');grid.replaceChildren();$('#photo-count').textContent=`${item.photos.length} / ${photoLimits.count}`;const sync=$('#sync-photos');sync.hidden=!pendingPhotos.size;sync.disabled=busy||uploading;
...
function setPhotoControls(disabled){$('#save-project').disabled=disabled||config.storage!=='cloudflare';$('#photo-upload').disabled=disabled;$('#camera-upload').disabled=disabled;$('#sync-photos').disabled=disabled||!pendingPhotos.size;}
...
async function initGoogle(){
  ...
  try{await new Promise((resolve,reject)=>{const callback='pisellMapsReady';let done=false;const finish=fn=>{if(done)return;done=true;clearTimeout(timer);fn();};const timer=setTimeout(()=>finish(()=>reject(new Error('地图加载超时，可先使用地点链接录入'))),15000);window[callback]=()=>finish(resolve);window.gm_authFailure=()=>{$('#location-message').textContent='Google Maps 验证失败，请检查地图配置；仍可手动录入位置';};const script=document.createElement('script');const url=new URL('https://maps.googleapis.com/maps/api/js');for(const [k,v] of Object.entries({key:config.googleMapsApiKey,loading:'async',callback,libraries:'places,maps,marker',v:'weekly',language:'zh-CN'}))url.searchParams.set(k,v);script.src=url;script.onerror=()=>finish(()=>reject(new Error('Google Maps 加载失败，可先使用地点链接录入')));document.head.append(script);});
    ...
    autocomplete.addEventListener('gmp-select',async({placePrediction})=>{const selection=++googleSelection;try{const place=placePrediction.toPlace();$('#location-message').textContent='正在读取地点…';await place.fetchFields({fields:['displayName','formattedAddress','location','addressComponents','googleMapsURI','photos']});if(selection!==googleSelection)return;if(!place.location)throw new Error('这个地点没有可用坐标，请选择其他结果');
      ...
      const code=place.addressComponents?.find(c=>c.types.includes('country'))?.shortText;if(countryContinents[code])item.continent=countryContinents[code];if(!item.title)item.title=place.displayName||'';placePhotos=(place.photos||[]).slice(0,10).flatMap(photo=>{try{const url=photo.getURI({maxWidth:900,maxHeight:600});return url?[{url,source:photo.googleMapsURI||place.googleMapsURI||url,credits:(photo.authorAttributions||[]).map(author=>({name:author.displayName||'Google Maps',url:author.uri||photo.googleMapsURI||place.googleMapsURI||url}))}]:[]}catch{return[];}});markDirty();fillFields();renderPlacePhotos();$('#location-message').textContent='地址、项目名称和坐标已填入，请核对门店信息';
    ...
    googlePin.addListener('dragend',()=>{const pos=googlePin.position;placePhotos=[];Object.assign(item,{lat:typeof pos.lat==='function'?pos.lat():pos.lat,lon:typeof pos.lng==='function'?pos.lng():pos.lng,googlePlaceId:'',googleMapsUrl:'',coordinateSource:'',locationNote:'位置已手动调整'});markDirty();fillFields();renderPlacePhotos();});renderLocation();
  }catch(error){$('#location-message').textContent=error.message;}
}
...
$('#case-form').onsubmit=async event=>{event.preventDefault();if(busy||uploading)return;if(config.storage!=='cloudflare'){$('#action-status').textContent='保存服务尚未连接，请完成 Cloudflare 配置';return;}const errors=caseErrors(item);showErrors(errors,true);if(Object.keys(errors).length)return;
  busy=true;setPhotoControls(true);$('.form-sections').inert=true;$('#save-project').textContent='正在保存…';
  try{
  ...
  finally{busy=false;setPhotoControls(false);$('#upload-progress').hidden=true;$('.form-sections').inert=false;$('#save-project').textContent='保存项目';}
};
...
async function openAdmin(){
  ...
  for(const f of fields){const input=$('#'+f.name);input.addEventListener('input',()=>{item[f.name]=numbers.has(f.name)?(input.value===''?null:Number(input.value)):input.value;
    if(['address','country','city'].includes(f.name)){placePhotos=[];item.lat=null;item.lon=null;item.googlePlaceId='';item.googleMapsUrl='';item.coordinateSource='';$('#lat').value='';$('#lon').value='';$('#google-link').value='';}
    if(['lat','lon'].includes(f.name)){placePhotos=[];item.googlePlaceId='';item.googleMapsUrl='';item.coordinateSource='';$('#google-link').value='';}
