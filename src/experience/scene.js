...
export function createAtlasScene(container,{onHover,onRegion,onCase}){
  ...
  function setData(items){
    ...
    // A pin represents one real address. City grouping hid individual project imagery and made the map feel like a directory.
    const displayItems=mapItems;
    for(const item of displayItems){
      ...
      const label=document.createElement('button');label.className='project-pin';label.setAttribute('aria-label',`View project ${item.title}`);const thumbnail=document.createElement('img');thumbnail.src=item.cover||'';thumbnail.alt='';thumbnail.loading='lazy';const copy=document.createElement('span'),title=document.createElement('b'),place=document.createElement('small'),count=document.createElement('i');title.textContent=item.title;place.textContent=[item.city,item.country].filter(Boolean).join(' · ');count.textContent=String(item.photos?.length||0).padStart(2,'0');copy.append(title,place);label.append(thumbnail,copy,count);label.onclick=()=>onCase(item.id);label.hidden=true;labels.append(label);pins.push({item,group,label,elevation});
    }
  }
